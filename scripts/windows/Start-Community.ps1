...
Write-Host ""
Write-Host "BSC DLP Community v0.5.4" -ForegroundColor Magenta
...
function Test-BscHealth {
    try {
        ...
        return ($h.status -eq "ok" -and $h.engine -eq "BSC DLP" -and $h.version -eq "0.5.4")
    }
    catch { return $false }
}
...
Write-Host ""
Write-Host "BSC DLP v0.5.4 is running." -ForegroundColor Green
