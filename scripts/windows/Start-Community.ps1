...
Write-Host ""
Write-Host "BSC DLP Community v0.6.2" -ForegroundColor Magenta
...
function Test-BscHealth {
    try {
        ...
        return ($h.status -eq "ok" -and $h.engine -eq "BSC DLP" -and $h.version -eq "0.6.2")
    }
    catch { return $false }
}
...
Write-Host ""
Write-Host "BSC DLP v0.6.2 is running." -ForegroundColor Green
