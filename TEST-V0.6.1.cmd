...
echo.
echo BSC DLP v0.6.1 validation
echo.
...
if errorlevel 1 (
  ...
  pushd agent
  go build -o "%TEMP%\bsc-dlp-agent-v061-test.exe" .
  if errorlevel 1 (
    popd
    goto :fail
  )
  del /q "%TEMP%\bsc-dlp-agent-v061-test.exe" >nul 2>&1
  popd
)
...
echo Restart BSC DLP, then run TEST-WHATSAPP-DLP.cmd.
pause
exit /b 0
